...
const Header = () => {
  ...
  return (
      ...
      <div className='hidden items-center gap-5 text-[#FAF9F6] sm:flex lg:gap-10'>
        ...
        <div className='relative cursor-pointer' onClick={() => setShowMenu(!showMenu)}>
          <div className='flex items-center capitalize'>
            Courses <IoIosArrowDown className='pl-2' size={15}/>
          </div>
          {showMenu && (
            <div className='my absolute right-0 mt-20 w-48 rounded-lg bg-white text-black'>
    ...
    <div className={`flex left-0 top-0 z-40 h-full w-full bg-main print-hidden ${!open && "hidden"}`}>
      ...
      <div className='m-auto mt-20 flex w-[90%]  flex-col text-base font-normal text-white'>
        ...
        <div className={`py-5 ${!showMenu && "border-b"} border-[#146895]`} onClick={() => 
          ...
          <div className={`overflow-y-auto rounded-xl bg-white ${showMenu ? "h-auto" : "h-0 overflow-hidden"}`}>
            <h1 className='mt-3 px-3 text-left text-lg font-bold text-zinc-800'>Core Courses</h1>
          {CompulsoryData.map((val, index) => (
            <Link onClick={() => setOpen(false)} key={index} href={`/compulsory/${val.id}`}>
              <div className='py-3 text-sm text-zinc-800 sm:text-base hover:text-main ml-5'>{val.text}</div>
            </Link>
          ))}
          <h1 className='mt-3 px-2 text-left text-lg font-bold text-zinc-800'>Advance Courses</h1>
          {tracks.map((val, index) => (
            <Link onClick={() => setOpen(false)} key={index} href={`/tracks/${val.id}`}>
              <div className='py-3 text-sm text-zinc-800 sm:text-base hover:text-main ml-5'>{val.name}</div>
            </Link>
          ))}
          </div>
      </div>
